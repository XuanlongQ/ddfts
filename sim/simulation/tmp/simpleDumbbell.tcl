...
proc finish {} {
        global ns enableNAM namfile mytracefile throughputfile f
        $ns flush-trace
        close $f
        close $mytracefile
...
#NAM设置:链路颜色为绿色，nqueue到nclient之间的变化
#queePos or position of queue is defined as the angle of the queue line with horizontal
$ns duplex-link-op $nqueue $nclient color "green"
$ns duplex-link-op $nqueue $nclient queuePos 0.25
#inserts objects that allows us to monitor the queue size. This returns
#a handle to the object that may be queried to determine the average queue size..
#The default value for sampleinterval is 0.1
#监视队列变化情况
...
#将监视对象$flowmon与被监视链路$MainLink关联
$ns attach-fmon $MainLink $flowmon

#Classifiter/Hash/Fid
